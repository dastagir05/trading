      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <div className={`px-3 py-1 ${getHealthColor(stats?.systemHealth || 'good')} text-sm font-medium rounded-full`}>
              {stats?.systemHealth === 'excellent' && <CheckCircle className="w-4 h-4 inline mr-1" />}
              {stats?.systemHealth === 'good' && <CheckCircle className="w-4 h-4 inline mr-1" />}
              {stats?.systemHealth === 'warning' && <AlertTriangle className="w-4 h-4 inline mr-1" />}
              {stats?.systemHealth === 'critical' && <AlertTriangle className="w-4 h-4 inline mr-1" />}
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-orange-600">Server Load</p>
                <p className="text-2xl font-bold text-orange-900">{stats?.serverLoad}%</p>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-gray-900">{stats?.databaseConnections}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">API Requests (24h)</span>
                    <span className="text-sm font-semibold text-gray-900">{stats?.apiRequests}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Error Rate</span>
                    <span className="text-sm font-semibold text-gray-900">{(stats?.errorRate || 0) * 100}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-gray-600">API rate limit warning</span>
