...
const setupAdminSystem = async () => {
  try {
    ...
    const existingAdmin = await Admin.findOne({ email: 'pinjaridastageer@gmail.com' });
    ...
    const sampleLogs = [
      ...
      },
      {
        ...
        riskLevel: 'low',
        details: { 
          changes: ['admin_models_created', 'logging_enabled', 'permissions_set']
        }
      },
      {
        adminId: admin._id,
    ...
    if (userWithAdminRole) {
      ...
      console.log('ℹ️  User model supports admin role (no users with admin role yet)');
    }
    ...
    console.log('   GET  /api/admin-auth/activity/:adminId - Get activity summary');
