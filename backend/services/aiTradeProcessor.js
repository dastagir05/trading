...
class AiTradeProcessor {
  ...
  async processSuggestion(suggestion) {
    try {
      ...
      await aiTrade.save();
      console.log(`📝 New AI trade created: ${suggestion.title}`);
  ...
  async updateSuggestion(existingTrade, suggestion) {
    try {
      ...
      console.log(`🔄 Updated AI trade: ${suggestion.title}`);
    } catch (error) {
  ...
  async checkTradeStatus(trade) {
    try {
      ...
      if (trade.expiryDate && now > trade.expiryDate) {
        ...
        console.log(`⏰ AI trade expired: ${trade.title}`);
        return;
      }
      ...
      console.error(`❌ Error checking trade status for ${trade.aiTradeId}:`, error);
    }
  }
  ...
  async activateTrade(trade) {
    try {
      ...
      console.log(`🚀 AI trade activated: ${trade.title}`);
    } catch (error) {
  ...
  async checkTargetStopLoss(trade) {
    try {
      ...
      // const entryPrice = parseFloat(trade.tradePlan.entry.replace('₹', ''));
      // const targetPrice = parseFloat(trade.tradePlan.target.replace('₹', ''));
      // const stopLossPrice = parseFloat(trade.tradePlan.stopLoss.replace('₹', ''));
      
      // if (currentPrice >= targetPrice) {
      //   await trade.updateStatus('target_hit', 'Target price achieved');
      //   await trade.addNote(`Target hit at ₹${currentPrice}`, 'success');
      // } else if (currentPrice <= stopLossPrice) {
      //   await trade.updateStatus('stoploss_hit', 'Stop loss triggered');
      //   await trade.addNote(`Stop loss hit at ₹${currentPrice}`, 'warning');
      // }
      
    } catch (error) {
      console.error(`❌ Error checking target/stop loss for ${trade.aiTradeId}:`, error);
    }
  }
