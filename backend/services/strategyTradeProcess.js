...
class StrategyTradeProcessor {
  ...
  determineTradeType(aiTrade) {
    ...
    const sellIndicators = ['sell', 'short', 'write', 'covered call', 'cash secured put', 'iron condor', 'iron butterfly'];
    const titleLower = aiTrade.title?.toLowerCase() || '';
    const strategyLower = aiTrade.setup?.strategy?.toLowerCase() || '';
    
    const isSellStrategy = sellIndicators.some(indicator => 
  ...
  async checkStrategyForActivation(strategy) {
    try {
      ...
      if (activeTrades.length > 0) {
        console.log(`🔥 Found ${activeTrades.length} active trades, activating all ${pendingTrades.length} pending trades immediately`);
      ...
      for (const trade of pendingTrades) {
        ...
        if (shouldActivate) {
          ...
          const otherPendingTrades = pendingTrades.filter(t => t.tradeId !== trade.tradeId);
          if (otherPendingTrades.length > 0) {
            console.log(`🚀 First trade activated, immediately activating ${otherPendingTrades.length} other trades in strategy`);
  ...
  async checkTradeEntryConditions(trade) {
    try {
      ...
      const tolerancePercent = trade.tradeType === 'SELL' ? 0.15 : 0.10; // Higher tolerance for SELL
  ...
  async activateStrategyTrade(strategy, trade) {
    try {
      ...
      if (!currentPrice) {
        console.log(`⚠️ Cannot get current price for ${trade.setup.strike}, skipping activation`);
        return false;
      }
      ...
      console.log(`🚀 Strategy trade activated: ${trade.setup.strike} (${trade.tradeType}) at ₹${currentPrice}`);
      return true;
    } catch (error) {
  ...
  async handleExpiredTrade(strategy, trade) {
    try {
      ...
      const pnl = this.calculatePnL(trade.entryPrice, currentPrice, trade.sentiment, trade.tradeType);
  ...
  async checkTradeTargetStopLoss(strategy, trade) {
    try {
      ...
      if (tradeType === 'SELL') {
        ...
        console.log(`🔍 SELL Trade Check - Current: ₹${currentPrice}, Target: ₹${targetPrice}, SL: ₹${stopLossPrice}`);
      } else {
        ...
        console.log(`🔍 BUY Trade Check (${trade.sentiment}) - Current: ₹${currentPrice}, Target: ₹${targetPrice}, SL: ₹${stopLossPrice}`);
      }
  ...
  async handleTargetHit(strategy, trade, currentPrice) {
    ...
    await strategy.addNote(
      `Trade ${trade.tradeId} (${trade.tradeType}) target hit at ₹${currentPrice}. P&L: ₹${pnl.toFixed(2)}`,
      "success"
    );

    // Exit all other active trades in the strategy
    await this.exitAllOtherActiveTrades(strategy, trade.tradeId, "Target hit on another trade");
  }
  ...
  async handleStopLossHit(strategy, trade, currentPrice) {
    ...
    await strategy.addNote(
      `Trade ${trade.tradeId} (${trade.tradeType}) stop loss hit at ₹${currentPrice}. P&L: ₹${pnl.toFixed(2)}`,
      "warning"
    );

    // Exit all other active trades in the strategy
    await this.exitAllOtherActiveTrades(strategy, trade.tradeId, "Stop loss hit on another trade");
  }
  ...
  async exitAllOtherActiveTrades(strategy, excludeTradeId, exitReason) {
    try {
      ...
      console.log(`🔄 Exiting ${otherActiveTrades.length} other active trades due to: ${exitReason}`);
      ...
      for (const trade of otherActiveTrades) {
        try {
          ...
          if (!currentPrice) {
            console.log(`⚠️ Cannot get current price for ${trade.setup.strike}, skipping exit`);
            continue;
          }
          ...
          await strategy.addNote(
            `Trade ${trade.tradeId} (${trade.tradeType}) exited at ₹${currentPrice} due to ${exitReason}. P&L: ₹${pnl.toFixed(2)}`,
            "info"
          );

          console.log(`✅ Exited trade ${trade.tradeId} (${trade.tradeType}) at ₹${currentPrice}`);
  ...
  async updateStrategyStatus(strategy) {
    try {
      ...
      const completedTrades = strategy.trades.filter(t => 
        ["target_hit", "stoploss_hit", "strategy_exit", "expired"].includes(t.status)
      );
      ...
      if (activeTrades.length === 0 && completedTrades.length > 0) {
        ...
        const totalCharges = this.calculateStrategyTotalCharges(strategy.trades);
        ...
        // Update parent AiTrade status
        await this.updateAiTradeStatus(strategy.strategyId, "completed", netPnL);

        console.log(`📊 Strategy ${strategy.strategyId} completed with total P&L: ₹${totalPnL.toFixed(2)}, Net P&L: ₹${netPnL.toFixed(2)}`);
  ...
  calculateTradeCharges(exitPrice, trade) {
    ...
    const percentPnL = trade.entryPrice > 0 ? 
      parseFloat(((grossPnl / (trade.entryPrice * trade.quantity)) * 100).toFixed(2)) : 0;
    ...
    console.log(`💰 Trade charges calculated - Gross P&L: ₹${grossPnl.toFixed(2)}, Net P&L: ₹${netPnl.toFixed(2)}, Charges: ₹${totalCharges}`);
  }
  ...
  async updateAiTradeStatus(strategyId, status, totalPnL = null) {
    try {
      ...
      if (aiTrade.status !== status) {
        ...
        console.log(`Updated parent AiTrade ${strategyId} status to: ${status}`);
      }
    } catch (error) {
  ...
  validateTradePlanValues(instruments, entryValues, targetValues, stopLossValues) {
    ...
    if (entryValues.length !== instrumentCount) {
      console.warn(`⚠️ Entry values count (${entryValues.length}) doesn't match instruments count (${instrumentCount})`);
    }
    
    if (targetValues.length !== instrumentCount) {
      console.warn(`⚠️ Target values count (${targetValues.length}) doesn't match instruments count (${instrumentCount})`);
    }
    
    if (stopLossValues.length !== instrumentCount) {
      console.warn(`⚠️ StopLoss values count (${stopLossValues.length}) doesn't match instruments count (${instrumentCount})`);
    }
  }
