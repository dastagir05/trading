...
const aiTradeProcessor = require('./aiTradeProcessor');
...
class AiTradeIntegration {
  ...
  async convertAiTradeToTrade(aiTradeId, userId, tradeData) {
    try {
      ...
      const newTrade = new Trades({
        userId,
        symbol: aiTrade.setup.symbol || this.extractSymbol(aiTrade.setup.strike),
        instrumentKey: aiTrade.setup.instrumentKey || this.generateInstrumentKey(aiTrade),
        quantity: tradeData.quantity || 1,
        entryPrice: parseFloat(aiTrade.tradePlan.entry.replace('₹', '').replace(',', '')),
        ...
        stoploss: parseFloat(aiTrade.tradePlan.stopLoss.replace('₹', '').replace(',', '')),
        target: parseFloat(aiTrade.tradePlan.target.replace('₹', '').replace(',', '')),
      ...
      console.log(`🔄 AI trade ${aiTradeId} converted to trade ${newTrade._id} for user ${userId}`);
  ...
  async updateAiTradeFromTrade(tradeId, newStatus, exitPrice = null) {
    try {
      ...
      if (aiTradeStatus !== aiTrade.status) {
        ...
        await aiTrade.addNote(`Trade ${newStatus} - Exit price: ₹${exitPrice || 'N/A'}`, 'info');
        
        console.log(`🔄 AI trade ${aiTrade.aiTradeId} status updated to ${aiTradeStatus}`);
      }
  ...
  async getUserAiTradePerformance(userId) {
    try {
      ...
      userAiTrades.forEach(trade => {
        ...
        } else if (['target achieve', 'stoploss hit', 'expired', 'user exited manually'].includes(trade.status)) {
      ...
      if (performance.completedTrades > 0) {
        performance.winRate = (winningTrades / performance.completedTrades) * 100;
      }
  ...
  async getPersonalizedAiTradeSuggestions(userId, preferences = {}) {
    try {
      ...
      console.error(`❌ Error getting personalized AI trade suggestions:`, error);
      throw error;
    }
  }
