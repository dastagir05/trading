  ...
  try {
    ...
    if (!email || !provider || !providerId) {
      await AdminLog.logAction(
        null,
        "login",
        "Admin login attempt with missing fields",
        {
    ...
    if (!admin) {
      ...
      if (admin._id && admin.adminId) {
        ...
        console.warn("Cannot log admin creation - missing _id or adminId:", {
          _id: admin._id,
          adminId: admin.adminId,
        });
      }
    ...
    if (admin.isLocked) {
      if (admin._id) {
        await AdminLog.logAction(
          admin._id,
          "login",
          "Login attempt on locked admin account",
          {
            adminEmail: email,
    ...
    if (!admin.isActive) {
      if (admin._id) {
        await AdminLog.logAction(
          admin._id,
          "login",
          "Login attempt on inactive admin account",
          {
            adminEmail: email,
            adminName: admin.name,
            ipAddress,
  ...
  try {
    ...
    if (adminId) {
      await AdminLog.logAction(
        adminId,
        "other",
        "Admin activity summary accessed",
        {
          adminEmail: admin.email,
          adminName: admin.name,
          ipAddress,
